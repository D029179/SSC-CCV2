...
<c:if test="${searchPageData.pagination.totalNumberOfResults == 0 && top && showTotals}">
    <div class="paginationBar top clearfix">
        <ycommerce:testId code="searchResults_productsFound_label">
			...
			<div class="backToSolution">
				<spring:url value="CPQCONFIGURATOR" var="navigateBackToConfigurationUrl"/>
				<form id="addRelatedProductForm-back" class="configure_form additional_button"
					action="${navigateBackToConfigurationUrl}" method="get">
					<button type="submit" class="btn btn-primary">
						<spring:theme text="Back to solution (not translated)" code="sapcustdev.solutionconfig.addrelatedproductback.button" />
					</button>
				</form>
			</div>
		    </c:if>
